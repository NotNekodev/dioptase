    ...
    fn step(vm: &mut VM, thread_ref: ThreadRef) -> Result<StepOutcome, RuntimeError> {
        ...
        match opcode {
            ...
            Opcode::IInc => {
                ...
                let local_var: &mut Value =
                    frame
                        ...
                        .ok_or(InternalError::NoLocalVar {
...

                {
                    ...
                    if index >= len {
                        return Err(vm.throw(
                            ...
                            Some(&format!("Index {} out of bounds for length {}", index, len)),
                        ));
                    }
                }
            ...
            Opcode::InvokeVirtual => {
                ...
                let mut new_frame = Frame::new(max_locals, max_stack, resolved_class, method_idx);
